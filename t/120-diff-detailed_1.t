...
ok($d = diff($a, $b, 'detailed' => 1) and
    ...
    keys %{$d->{'D'}->[0]} == 1 and exists $d->{'D'}->[0]->{'U'} and $d->{'D'}->[0]->{'U'} == 0 and
    keys %{$d->{'D'}->[1]} == 1 and exists $d->{'D'}->[1]->{'D'} and @{$d->{'D'}->[1]->{'D'}} == 1 and
        exists $d->{'D'}->[1]->{'D'}->[0]->{'D'} and @{$d->{'D'}->[1]->{'D'}->[0]->{'D'}} == 1 and
        keys %{$d->{'D'}->[1]->{'D'}->[0]->{'D'}->[0]} == 1 and exists $d->{'D'}->[1]->{'D'}->[0]->{'D'}->[0]->{'U'} and
        $d->{'D'}->[1]->{'D'}->[0]->{'D'}->[0]->{'U'} == 100 and
